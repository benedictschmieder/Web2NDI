...
function main() {
  ...
  if (electronVersion) {
    ...
    console.log(`[build-native] Building addon for Electron ${electronVersion} ...`);
  } else {
    console.log('[build-native] Electron not found, building for system Node.js ...');
  }
  ...
  try {
    execFileSync(bin, args, { stdio: 'inherit', env, shell: process.platform === 'win32' });
  } catch (err) {
    console.error('\n[build-native] Native build failed.');
    console.error('[build-native] Make sure the following are installed:');
    console.error('  - Visual Studio Build Tools with "Desktop development with C++"');
    console.error('  - NDI 6 SDK (default: C:\\Program Files\\NDI\\NDI 6 SDK)');
    console.error('    or set the NDI_SDK_DIR environment variable to its location.');
    process.exit(1);
  }
}
